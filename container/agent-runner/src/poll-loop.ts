  ...
  try {
    for await (const event of query.events) {
      ...
      if (event.type === 'init') {
        ...
        if (event.text) {
          const { hasUnwrapped } = dispatchResultText(event.text, routing, assistantName);
 ...
 */
function dispatchResultText(text: string, routing: RoutingContext, assistantName?: string): { sent: number; hasUnwrapped: boolean } {
  ...
  // otherwise fall back to the single destination. Needed for Gemini which
  // doesn't use <message> wrapping.
