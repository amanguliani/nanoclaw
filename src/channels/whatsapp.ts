...
registerChannelAdapter('whatsapp', {
  factory: () => {
    const env = readEnvFile(['WHATSAPP_PHONE_NUMBER', 'WHATSAPP_ENABLED', 'WHATSAPP_ALLOWED_GROUPS']);
    const phoneNumber = env.WHATSAPP_PHONE_NUMBER;
    const allowedGroups = new Set(
      (env.WHATSAPP_ALLOWED_GROUPS || '')
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean),
    );
      ...
      sock.ev.on('messages.upsert', async ({ messages }) => {
        for (const msg of messages) {
          try {
            ...
            if (isGroup && allowedGroups.size > 0 && !allowedGroups.has(chatJid)) {
              log.debug('Skipping message from non-allowlisted WhatsApp group', { chatJid });
              continue;
            }
